...
with open("_itartass1.xhtml", "r", encoding = "utf-8") as file1, \
    ...
    with open("everything.xhtml", "r", encoding = "utf-8") as file:
        for line in file.readlines():
            if "docid" in line:
                docid = re.findall(r'[А-ЯЁA-Zа-яё]"', line)
            elif "title" in line:
                title = re.findall(r'[А-ЯЁA-Zа-яё]"', line)
            elif "author" in line:
                author = re.findall(r'[А-ЯЁA-Zа-яё]"', line)
            elif "created" in line:
                created = re.findall(r'[\d]"', line)
            elif "topic" in line:
                topic = re.findall(r'[А-ЯЁA-Zа-яё]"', line)
            elif "tagging" in line:
                tagging = re.findall(r'[А-ЯЁA-Zа-яё]"', line)
